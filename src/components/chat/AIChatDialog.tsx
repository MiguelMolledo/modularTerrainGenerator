...
export function AIChatDialog() {
  ...
  const formatToolResult = (result: unknown, name: string): React.ReactNode => {
        ...
        <div className="mt-2 space-y-2">
          <div className="bg-amber-900/30 border border-amber-700/50 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-medium text-foreground">{terrain.name}</span>
              <span
            ...
            {terrain.description && (
              <p className="text-xs text-muted-foreground mb-2">{terrain.description}</p>
            )}
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-1">
                  {terrain.pieces.map((piece, i) => (
                    <div key={i} className="flex items-center justify-between bg-card px-2 py-1 rounded text-xs">
                      <span className="text-foreground">{piece.shapeName}</span>
        ...
        return (
          <div className="mt-2 text-sm">
            ...
            {Number(r.piecesAssigned) > 0 && (
              <p className="text-muted-foreground text-xs mt-1">{Number(r.piecesAssigned)} piece types assigned</p>
            )}
            {shapesCreated.length > 0 && (
              <p className="text-muted-foreground text-xs">Shapes created: {shapesCreated.join(', ')}</p>
            )}
          </div>
        );
      }
    ...
    if (name === 'list_maps' && r.maps) {
      ...
      return (
        <div className="mt-2 text-sm">
          <span className="text-primary">{Number(r.count)} maps found:</span>
          <div className="space-y-1 mt-2">
            {maps.slice(0, 8).map((map) => (
              <div key={map.id} className="flex items-center justify-between bg-card px-2 py-1 rounded text-xs">
                <span className="text-foreground">🗺️ {map.name}</span>
                <span className="text-muted-foreground">{map.dimensions} • {map.piecesCount} pieces</span>
              </div>
            ))}
            {maps.length > 8 && <span className="text-muted-foreground text-xs">+{maps.length - 8} more</span>}
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="mt-2 text-sm">
          <div className="bg-primary/10 border border-primary/30 rounded-lg p-3">
            ...
            {terrains.length > 0 && (
              <div className="mb-2">
                <span className="text-xs text-primary">Terrains:</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {terrains.map((t, i) => (
                    <span key={i} className="bg-card px-2 py-0.5 rounded text-xs">
            ...
            {props.length > 0 && (
              <div>
                <span className="text-xs text-primary">Props:</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  ...
                  {props.length > 6 && <span className="text-muted-foreground text-xs">+{props.length - 6} more</span>}
                </div>
              </div>
            )}
          </div>
      ...
      return (
        <div className="space-y-3 mt-2">
          <div className="bg-indigo-900/30 border border-indigo-700/50 rounded-lg p-3">
            ...
            <p className="text-sm text-foreground italic whitespace-pre-wrap">{readAloud}</p>
          </div>
          {dmNotes && (
            <div className="bg-card border border-border rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-muted-foreground">DM Notes</span>
                <button
              ...
              <p className="text-sm text-foreground whitespace-pre-wrap">{dmNotes}</p>
            </div>
          )}
        </div>
      );
      ...
      return (
        <div className="mt-2 text-sm">
          ...
          <div className="grid grid-cols-1 gap-1 mt-2">
            {props.map((prop) => (
              <div key={prop.index} className="flex items-center gap-2 bg-card px-2 py-1.5 rounded text-xs">
                <span className="text-muted-foreground">{prop.index + 1}.</span>
                <span className="text-lg">{prop.emoji}</span>
                <span className="text-foreground">{prop.name}</span>
                <span className="text-muted-foreground ml-auto">{prop.category}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      return (
        <div className="mt-2 text-sm">
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            ...
            {props.length > 10 && <span className="text-muted-foreground text-xs">+{props.length - 10} more</span>}
          </div>
        </div>
      );
    }
  ...
  return (
    <>
      ...
      {/* Chat Panel */}
      {isOpen && !isMinimized && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 w-[500px] max-w-[calc(100vw-2rem)] bg-background border border-border rounded-xl shadow-2xl shadow-black/50 flex flex-col max-h-[500px]">
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[200px]">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center space-y-4 py-4">
                ...
                <div>
                  <h3 className="text-lg font-medium text-foreground mb-1">Hi! I&apos;m your assistant</h3>
                ...
                <div className="w-full space-y-1.5">
                  <button
                    className="w-full text-left px-3 py-2 bg-card/80 hover:bg-accent rounded-lg text-sm border border-border hover:border-purple-500/50 transition-colors"
                    onClick={() => sendMessage('I want to create a new piece')}
                    disabled={isLoading}
                  >
                    <span className="text-purple-400">🧩 Create Piece</span>
                    <span className="text-muted-foreground ml-2">- Guided step by step</span>
                  </button>
                  <button
                    className="w-full text-left px-3 py-2 bg-card/80 hover:bg-accent rounded-lg text-sm border border-border hover:border-purple-500/50 transition-colors"
                  ...
                  >
                    ...
                    <span className="text-muted-foreground ml-2">- Forest, snow, lava...</span>
                  </button>
                  <button
                    className="w-full text-left px-3 py-2 bg-card/80 hover:bg-accent rounded-lg text-sm border border-border hover:border-purple-500/50 transition-colors"
                  ...
                  >
                    ...
                    <span className="text-muted-foreground ml-2">- NPCs, furniture...</span>
                  </button>
                  <button
                    className="w-full text-left px-3 py-2 bg-card/80 hover:bg-accent rounded-lg text-sm border border-border hover:border-purple-500/50 transition-colors"
                  ...
                  >
                    ...
                    <span className="text-muted-foreground ml-2">- Read aloud text</span>
                  </button>
                </div>
              </div>
            ) : (
              <>
                ...
                {error && (
                  <div className="flex justify-center">
                    <div className="bg-destructive/20 text-destructive rounded-lg px-3 py-2 flex items-center gap-2 text-sm">
          ...
          >
            <input
              ...
              className="flex-1 bg-card border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-purple-500"
              disabled={isLoading}
            />
            <Button
              type="submit"
